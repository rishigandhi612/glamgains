...
export const Reviews = ({ productId, averageRating }) => {
  ...
  return (
    <Stack rowGap={5} alignSelf={"flex-start"} width={is480 ? "90vw" : is840 ? "25rem" : "40rem"}>
        ...
        <Typography gutterBottom variant='h4' fontWeight={400}>Reviews</Typography>
        {
          reviews?.length ? (
            ...
            <Typography variant='h6' color={'text.secondary'} fontWeight={400}>
              {loggedInUser?.isAdmin ? "There are no reviews currently" : "Be the one to post a review first"}
            </Typography>
          )
        }
      </Stack>
      ...
      <Stack rowGap={2}>
        {reviews?.map((review) => {
          // Check if review and review.user are not null before passing to ReviewItem
          if (review?.user) {
            return (
              <ReviewItem
                key={review._id}
                id={review._id}
                userid={review.user?._id}  // Null-safe access to user._id
                comment={review.comment}
                createdAt={review.createdAt}
                rating={review.rating}
                username={review.user?.name}  // Null-safe access to user.name
              />
            );
          } else {
            return null; // If review or review.user is null, return nothing
          }
        })}
      </Stack>
          ...
          <Stack rowGap={3} position={'relative'} component={'form'} noValidate onSubmit={handleSubmit(handleAddReview)}>
            ...
            <Stack>
              ...
              <motion.div style={{ width: "fit-content" }} whileHover={{ scale: 1.05 }} whileTap={{ scale: 1 }}>
                <Rating size='large' value={value} onChange={(e, newValue) => setValue(newValue)} />
              </motion.div>
            </Stack>
