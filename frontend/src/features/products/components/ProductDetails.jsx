...
export const ProductDetails = () => {
    ...
    const handleToast = (status, successMessage, errorMessage) => {
        if (status === 'fulfilled') {
      ...
      useEffect(() => {
        handleToast(cartItemAddStatus, "Product added to cart", "Error adding product to cart");
    }, [cartItemAddStatus]);
    
    useEffect(() => {
        handleToast(wishlistItemAddStatus, "Product added to wishlist", "Error adding product to wishlist");
    }, [wishlistItemAddStatus]);
    
    useEffect(() => {
        handleToast(wishlistItemDeleteStatus, "Product removed from wishlist", "Error removing product from wishlist");
    ...
    const handleIncreaseQty = () => {
        if (quantity < Math.min(product?.stockQuantity, 20)) {
            setQuantity(quantity + 1);
        }
    };    
    ...
    const maxSteps = product?.images?.length ?? 0;  // Default to 0 if images are undefined or null
                ...
                {!is1420 && (
                  <Stack sx={{ display: "flex", rowGap: '1.5rem', height: "100%", overflowY: "scroll" }}>
                    {product?.images.map((image, index) => (
                      <motion.div key={image} whileHover={{ scale: 1.1 }} whileTap={{ scale: 1 }} style={{ width: "200px", cursor: "pointer" }} onClick={() => setSelectedImageIndex(index)}>
                ...
                <Stack mt={is480 ? "0rem" : '5rem'}>
                  {is1420 ? (
                    <Stack width={is480 ? "100%" : is990 ? '400px' : "500px"}>
                      ...
                      >
                        {(product?.images ?? []).map((image, index) => (
                          <div key={image} style={{ width: "100%", height: "100%" }}>
                            <Box
                              component="img"
                              sx={{
                                width: '100%',
                      ...
                      {/* Render MobileStepper only if maxSteps is greater than 0 */}
                      {maxSteps > 0 && (
                        <MobileStepper
                          steps={maxSteps}
                          position="static"
                    ...
                    <Stack flexDirection={'row'} alignItems={'center'} columnGap={is387 ? '5px' : '1rem'} width={'fit-content'}>
                      ...
                      <Stack flexDirection={'row'} columnGap={is387 ? ".5rem" : ".2rem"}>
                        {COLORS.map((color, index) => (
                          <div key={color} style={{ backgroundColor: "white", border: selectedColorIndex === index ? `1px solid ${theme.palette.primary.dark}` : "", width: is340 ? "40px" : "50px", height: is340 ? "40px" : "50px", display: "flex", justifyContent: "center", alignItems: "center", borderRadius: "100%" }}>
                    ...
                    <Stack flexDirection={"row"} columnGap={is387 ? ".3rem" : "1.5rem"} width={'100%'} >
                      <Stack flexDirection={'row'} alignItems={'center'} justifyContent={'space-between'}>
                        <MotionConfig whileHover={{ scale: 1.050 }} whileTap={{ scale: 1 }}>
                          <motion.button aria-label="Decrease quantity" onClick={handleDecreaseQty} style={{ padding: "10px 15px", fontSize: "1.050rem", backgroundColor: "", color: "black", outline: "none", border: '1px solid black', borderRadius: "8px" }}>-</motion.button>
                          <p style={{ margin: "0 1rem", fontSize: "1.1rem", fontWeight: '400' }}>{quantity}</p>
                          <motion.button aria-label="Increase quantity" onClick={handleIncreaseQty} style={{ padding: "10px 15px", fontSize: "1.050rem", backgroundColor: "black", color: "white", outline: "none", border: 'none', borderRadius: "8px" }}>+</motion.button>
                        </MotionConfig>
                      </Stack>
                      {/* Add to cart */}
